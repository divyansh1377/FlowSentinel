...
def test_phase2_capabilities():
    ...
    print(f"  • Risk Score: {p1['diagnostic_breakdown']['chute_health_index']['risk_score']}/100")
    print(f"  • Health State: {p1['diagnostic_breakdown']['chute_health_index']['health_state']}")
    ...
    time.sleep(0.05)
    predictor.predict({"distance_cm": 40.0, "weight_kg": 450.0, "vibration_g": 2.5, "material_flow_rate_tph": 180.0})
    time.sleep(0.05)
    p3 = predictor.predict({"distance_cm": 25.0, "weight_kg": 650.0, "vibration_g": 1.4, "material_flow_rate_tph": 120.0})
    ...
    print(f"  • Fault Category:                {p3['diagnostic_breakdown']['fault_category']}")
    ...
    print(f"  • Risk Score: {p_choke['diagnostic_breakdown']['chute_health_index']['risk_score']}/100")
    ...
    assert p_choke["diagnostic_breakdown"]["fault_category"] in ["CRITICAL_FUNNEL_CHOKE", "OVERBURDEN_FLOW_STOPPAGE", "RAPID_SURGE_JAM"]
