...
def train_and_export_models(n_samples: int = 20000):
    ...
    print(
        "🧠 [Team 3 ML] Training Isolation Forest for Unsupervised Anomaly Detection..."
    )
    ...
    anomaly_threshold = float(
        np.quantile(calibration_scores, IF_CALIBRATION_FALSE_POSITIVE_RATE)
    )

    # Step 2: Train Random Forest Classifier on non-anomalous labeled data
    print(
        "🌲 [Team 3 ML] Training Random Forest for 3-State Chute Classification (0=Normal, 1=Warning, 2=Blockage)..."
    )
    ...
    report = classification_report(
        y_test, y_pred, target_names=["NORMAL", "WARNING", "BLOCKAGE"], output_dict=True
    )
    ...
    print(
        classification_report(
            y_test, y_pred, target_names=["NORMAL", "WARNING", "BLOCKAGE"]
        )
    )
    ...
    importances = dict(
        zip(
            FEATURE_COLUMNS,
            [round(float(v), 4) for v in random_forest.feature_importances_],
        )
    )
    ...
    metadata = {
        ...
        "classes": {0: "NORMAL", 1: "WARNING", 2: "BLOCKAGE"},
    }
