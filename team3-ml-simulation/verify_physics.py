...
def run_physics_verification(n_samples: int = 20000):
    ...
    print(
        "  ✓ Sensor Physical Boundaries: All sensor readings within safe operational limits"
    )
    ...
    assert (
        corr_dist_weight < -0.75
    ), f"Expected strong negative correlation, got {corr_dist_weight}"
    print(
        "  ✓ PASS: Strong inverse correlation confirmed. As material accumulates, clearance strictly decreases."
    )
    ...
    print(
        f"  • State 0 (Normal Flow) Vibration Mean:      {norm_vib.mean():.3f} G (RMS)"
    )
    print(
        f"  • State 1 (Warning Buildup) Vibration Mean:  {warn_vib.mean():.3f} G (RMS)"
    )
    print(
        f"  • State 2 (Critical Blockage) Vibration Mean:{blk_vib.mean():.3f} G (RMS)"
    )

    assert (
        blk_vib.mean() < 0.50
    ), f"Expected blockage vibration < 0.50 G, got {blk_vib.mean():.3f}"
    assert (
        norm_vib.mean() > 2.50
    ), f"Expected normal vibration > 2.50 G, got {norm_vib.mean():.3f}"
    print(
        "  ✓ PASS: Kinetic dissipation confirmed. Flow stoppage deadens impact oscillations."
    )
    ...
    print(
        "  ✓ PASS: Tri-axial acceleration vectors mathematically agree with scalar RMS."
    )
    ...
    assert np.allclose(
        df["effective_accel_mps2"], expected_accel.clip(lower=0.05), atol=1e-4
    )
    ...
    print(
        "  ✓ Gravity/friction acceleration, bed height and velocity coupling verified"
    )
    ...
    print(
        "  ✓ Dropout, HX711 noise/drift, MPU6050 bias/jitter and quantization verified"
    )
    ...
    print(
        f"  • Class 0 (NORMAL):    {counts.get(0, 0)} ({counts.get(0, 0)/n_samples*100:.1f}%)"
    )
    print(
        f"  • Class 1 (WARNING):   {counts.get(1, 0)} ({counts.get(1, 0)/n_samples*100:.1f}%)"
    )
    print(
        f"  • Class 2 (BLOCKAGE):  {counts.get(2, 0)} ({counts.get(2, 0)/n_samples*100:.1f}%)"
    )
    print(
        f"  • Anomalies (IForest): {anomaly_count} ({anomaly_count/n_samples*100:.1f}%)"
    )
