...
def test_temporal_sequence_robustness():
    ...
    print(
        f"  • Frame 1 (Normal):    Status={r1['status_label']}, Risk={r1['diagnostic_breakdown']['chute_health_index']['risk_score']}/100"
    )
    ...
    print(
        f"  • Frame 2 (Spike):     Status={r2['status_label']}, Anomaly={r2['anomaly_detection']['is_anomaly']}, Risk={r2['diagnostic_breakdown']['chute_health_index']['risk_score']}/100"
    )
    ...
    print(
        f"  • Frame 3 (Recovery):  Status={r3['status_label']}, Risk={r3['diagnostic_breakdown']['chute_health_index']['risk_score']}/100"
    )
    ...
    print(
        "  ✓ PASS: Single-frame transient noise spike successfully absorbed without false blockage."
    )
    ...
    assert (
        risk1 < risk2 < risk3
    ), "Risk score MUST be strictly monotonic during progressive choke!"
