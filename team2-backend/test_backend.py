...
def test_get_presets():
    ...
    expected_presets = ["NORMAL_FLOW", "RISING_BUILDUP", "COMPLETE_BLOCKAGE", "EMPTY_CHUTE", "ERRATIC_SENSOR_SPIKE"]
...
def test_alert_lifecycle():
    ...
    client.post("/api/telemetry", json={
        "sensors": {"distance_cm": 4.0, "weight_kg": 1300.0, "vibration_g": 0.1}
    })
    ...
    # Acknowledge alert
    ack_resp = client.post(f"/api/alerts/{alert_id}/acknowledge", json={"acknowledged_by": "operator_1"})
...
def test_multiple_websocket_clients():
    ...
    with client.websocket_connect("/ws/telemetry") as ws1, client.websocket_connect("/ws/telemetry") as ws2:
        # Receive handshakes
        h1 = ws1.receive_json()
        h2 = ws2.receive_json()
